...
def _build_horoscope_prompt(day_configs: list[dict[str, str]]) -> str:
    ...
    return (
        ...
        f"Tone: vivid, witty, warm, natural, and lightly sarcastic. Keep every prediction broadly relatable regardless of age, income, relationship status, housing, or employment. "
        ...
        f"Distribute the signs across different universal experiences: mood, hesitation, a small choice, a conversation, rest, boundaries, curiosity, patience, a pleasant surprise, noticing something, and changing plans. "
        f"Never assume the reader has a job, colleagues, salary, rent, bank card, car, romantic partner, children, pets, a particular home, television, subscriptions, special clothing, or any named possession. Avoid brand names, purchases, fabricated financial events, and advice that requires owning something. "
        f"Every situation must be plausible and easy to recognize in ordinary life. Do not invent random surreal scenes, forced quirky objects, cartoonish coincidences, or absurd actions merely to sound funny. Humor must grow naturally from human behavior. "
        ...
        f"The text for each sign must be exactly ONE compact sentence of 10-16 words, containing a believable prediction and a subtle witty twist. Never exceed 16 words. "
        ...
        f"Vary the grammar: not every line may be advice or an imperative. Across one language block, use the words equivalent to 'today', 'simply/just', and 'instead of' no more than TWO times each. Do not begin more than two lines with the same grammatical pattern. "
        f"Avoid constructions equivalent to 'forget about', 'instead of doing X, do Y', 'your attempt will end with', and 'allow yourself'. "
